...
#            rs_id   consequence_term        ensembl_gene_id_cttv    gene_label
            ...
            regulatory=line[5]
            regulatory=regulatory.strip()
            if "regulatory" in regulatory:
               so_string = "regulatory_" + so_string 
            print "line0=  ", snp
            print "line1=  ", gene
    ...
    for line in csv.reader(results, delimiter="\t"):
        ...
        if not snpid in snpGeneMappings:
            ...
            for gene2so in snpGeneMappings[snpid]:
                geneidsString, so = gene2so.split(":")
                regulatory="false"
                #if "(regulatory)" in so: 
                #   regulatory="true"
                #   so="regulatory_region_variant"
                #   #so=so.replace("(regulatory)","")
                geneids = geneidsString.split(",")
